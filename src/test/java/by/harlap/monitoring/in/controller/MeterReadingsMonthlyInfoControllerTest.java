...
class MeterReadingsMonthlyInfoControllerTest {
    ...
    @BeforeEach
    public void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(meterReadingsMonthlyInfoController).build();
    }
    ...
    void doGetTest() throws Exception {
        ...
        when(meterReadingsMonthlyInfoFacade.createMeterReadingResponseSpecifiedByMonth(user,month,year)).thenReturn(records);

        final String expectedResponseBody = "[{\"date\":\"2024-01-01\",\"userId\":1,\"deviceId\":1,\"value\":100.2}]";

        mockMvc.perform(get("/meterReadingsForMonth")
                        .param("month",month)
                        .param("year", year)
