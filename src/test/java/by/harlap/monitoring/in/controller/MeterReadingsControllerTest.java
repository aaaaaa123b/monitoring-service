...
class MeterReadingsControllerTest {

    private MockMvc mockMvc;

    @Mock
    private MeterReadingsFacade meterReadingsFacade;
    @Mock
    ...
    @BeforeEach
    public void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(meterReadingsController).build();
    }
    ...
    void getHistoryOfMeterReadingsTest() throws Exception {
        ...
        when(meterReadingsFacade.findMeterReadingRecords(user)).thenReturn(records);
    ...
    void getMeterReadingsByMonthTest() throws Exception {
        ...
        when(securityUtil.findActiveUser(user.getUsername())).thenReturn(user);
        when(meterReadingsFacade.createMeterReadingResponseSpecifiedByMonth(user,month,year)).thenReturn(records);
    ...
    void getRelevantMeterReadingsTest() throws Exception {
        ...
        when(securityUtil.findActiveUser(user.getUsername())).thenReturn(user);
        when(meterReadingsFacade.createRelevantMeterReadingResponse(user)).thenReturn(records);
